...
use crate::dto::{CvResumeDto, CreateCvResumeDto, UpdateCvResumeDto, PageRequest, QsQuery};
...
/// Get all cvResumes with pagination
#[utoipa::path(
    get,
    path = "/api/cv-resumes",
    tag = "cv-resumes",
    ...
    responses(
        (status = 200, description = "List of cvResumes with pagination", body = Vec<CvResumeDto>,
            headers(
...
pub async fn get_all(
    ...
    let (items, total) = CvResumeService::find_all(&mut conn, &page_request)?;

    let dtos: Vec<CvResumeDto> = items.into_iter().map(CvResumeDto::from).collect();
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 401, description = "Unauthorized"),
        (status = 403, description = "Forbidden"),
        (status = 404, description = "CvResume not found")
    )
)]
pub async fn get_one(
    ...
    let mut conn = state.pool.get().map_err(|e| AppError::Internal(e.to_string()))?;
    let item = CvResumeService::find_by_id(&mut conn, id)?;

    Ok(Json(CvResumeDto::from(item)))
}
...
#[utoipa::path(
    ...
    tag = "cv-resumes",
    security(("bearer_auth" = [])),
    request_body = CreateCvResumeDto,
    responses(
        (status = 201, description = "CvResume created successfully", body = CvResumeDto),
...
pub async fn create(
    State(state): State<AppState>,
    Extension(auth_user): Extension<AuthUser>,
    Json(dto): Json<CreateCvResumeDto>,
) -> Result<(StatusCode, Json<CvResumeDto>), AppError> {
    if !auth_user.has_authority(RoleType::USER) {
        return Err(AppError::Forbidden("Access denied".to_string()));
    }

    let mut conn = state.pool.get().map_err(|e| AppError::Internal(e.to_string()))?;
    let item = CvResumeService::create(&mut conn, dto, &auth_user.login)?;


    Ok((StatusCode::CREATED, Json(CvResumeDto::from(item))))
}
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "CvResume updated successfully", body = CvResumeDto),
        (status = 400, description = "Invalid input"),
        (status = 401, description = "Unauthorized"),
        (status = 403, description = "Forbidden"),
        (status = 404, description = "CvResume not found")
...
pub async fn update(
    ...
    let mut conn = state.pool.get().map_err(|e| AppError::Internal(e.to_string()))?;
    let item = CvResumeService::update(&mut conn, id, dto, &auth_user.login)?;
...
pub async fn remove(
    ...
    let mut conn = state.pool.get().map_err(|e| AppError::Internal(e.to_string()))?;
    CvResumeService::delete(&mut conn, id)?;
...
mod integration_tests {
    ...
    #[tokio::test]
    async fn test_get_all_cvResumes() {
    ...
    #[tokio::test]
    async fn test_get_all_cvResumes_unauthorized() {
    ...
    #[tokio::test]
    async fn test_get_cvResume_not_found() {
        let (server, state) = create_test_app();
        create_test_user(&state.pool);
    ...
    #[tokio::test]
    async fn test_create_and_get_cvResume() {
        let (server, state) = create_test_app();
        create_test_user(&state.pool);
    ...
    async fn test_delete_cvResume() {
        ...
        let delete_response = server
            .delete(&format!("/api/cv-resumes/{}", id))
            .add_header("Authorization".parse().unwrap(), format!("Bearer {}", token).parse().unwrap())
            .await;
