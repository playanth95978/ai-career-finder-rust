...
use crate::db::schema::offer_tailored_resume;
...
impl OfferTailoredResumeService {
    /// Find all offerTailoredResumes with pagination
    pub fn find_all(
        conn: &mut DbConnection,
        page_request: &PageRequest,
    ) -> Result<(Vec<OfferTailoredResume>, i64), AppError> {
        let page = page_request.page.unwrap_or(0);
        let size = page_request.size.unwrap_or(20).min(100);
        let offset = page * size;

        let total: i64 = offer_tailored_resume::table
            .count()
            .get_result(conn)
        ...
        let results = match sort_field {
            "userId" | "user_id" => {
                if is_desc {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::user_id.desc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                } else {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::user_id.asc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                }
            ...
            "data" => {
                if is_desc {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::data.desc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                } else {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::data.asc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                }
            ...
            "title" => {
                if is_desc {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::title.desc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                } else {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::title.asc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                }
            ...
            "createdAt" | "created_at" => {
                if is_desc {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::created_at.desc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                } else {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::created_at.asc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                }
            ...
            "createdDate" | "created_date" => {
                if is_desc {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::created_date.desc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                } else {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::created_date.asc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                }
            ...
            "lastModifiedDate" | "last_modified_date" => {
                if is_desc {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::last_modified_date.desc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                } else {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::last_modified_date.asc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                }
            ...
            _ => {
                ...
                if is_desc {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::id.desc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                } else {
                    offer_tailored_resume::table
                        .order(offer_tailored_resume::id.asc())
                        .limit(size)
                        .offset(offset)
                        .select(OfferTailoredResume::as_select()).load(conn)
                }
        ...
        offer_tailored_resume::table
            ...
            .map_err(|e| match e {
                diesel::result::Error::NotFound => {
                    AppError::NotFound(format!("OfferTailoredResume {} not found", id))
                }
    ...
    pub fn create(
        conn: &mut DbConnection,
        dto: CreateOfferTailoredResumeDto,
        created_by: &str,
        ...
        let entity = diesel::insert_into(offer_tailored_resume::table)
            .values(&new_entity)
            .get_result::<OfferTailoredResume>(conn)
            .map_err(|e| AppError::Internal(e.to_string()))?;
    ...
    pub fn update(
        ...
        diesel::update(offer_tailored_resume::table.find(id))
            .set(&update)
            .execute(conn)
            .map_err(|e| AppError::Internal(e.to_string()))?;
...
mod tests {
    ...
    fn create_test_dto_with_suffix(suffix: u32) -> CreateOfferTailoredResumeDto {
    ...
    fn create_test_dto() -> CreateOfferTailoredResumeDto {
        let suffix = TEST_COUNTER.fetch_add(1, Ordering::SeqCst);
        create_test_dto_with_suffix(suffix)
    }

    #[test]
    fn test_create_offerTailoredResume() {
        let pool = create_test_pool();
        let mut conn = pool.get().unwrap();

        let dto = create_test_dto();
        let result = OfferTailoredResumeService::create(&mut conn, dto, "test_user");
    ...
    fn test_find_by_id() {
        ...
        let dto = create_test_dto();
        let created = OfferTailoredResumeService::create(&mut conn, dto, "test_user").unwrap();

        // Find it by ID
        let result = OfferTailoredResumeService::find_by_id(&mut conn, created.id);
        assert!(result.is_ok());
    ...
    fn test_find_all() {
        ...
        for _ in 0..3 {
            let dto = create_test_dto();
            OfferTailoredResumeService::create(&mut conn, dto, "test_user").unwrap();
        }
        ...
        let result = OfferTailoredResumeService::find_all(&mut conn, &page_request);
        assert!(result.is_ok());
    ...
    #[test]
    fn test_update_offerTailoredResume() {
        let pool = create_test_pool();
        let mut conn = pool.get().unwrap();
        ...
        // Update it
        let update_dto = UpdateOfferTailoredResumeDto {
        ...
        let result = OfferTailoredResumeService::update(&mut conn, created.id, update_dto, "test_user");
        assert!(result.is_ok());
    }

    #[test]
    fn test_delete_offerTailoredResume() {
        let pool = create_test_pool();
        let mut conn = pool.get().unwrap();
        ...
        // Delete it
        let result = OfferTailoredResumeService::delete(&mut conn, created.id);
        assert!(result.is_ok());

        // Verify it's gone
        let find_result = OfferTailoredResumeService::find_by_id(&mut conn, created.id);
