...
import autoApplyConfigResolve from './auto-apply-config-routing-resolve.service';

describe('AutoApplyConfig routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IAutoApplyConfig returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          autoApplyConfigResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(autoApplyConfigResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
