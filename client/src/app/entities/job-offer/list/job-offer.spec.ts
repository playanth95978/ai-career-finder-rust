...
import { sampleWithRequiredData } from '../job-offer.test-samples';
...
describe('JobOffer Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 9246 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.jobOffers()[0]).toEqual(expect.objectContaining({ id: 9246 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 9246 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.jobOffers()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 9246 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to jobOfferService', () => {
      ...
      vitest.spyOn(service, 'getJobOfferIdentifier');
      const id = comp.trackId(entity);
      expect(service.getJobOfferIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.jobOffersParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
