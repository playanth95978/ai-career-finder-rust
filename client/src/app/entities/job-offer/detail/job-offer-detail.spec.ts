...
describe('JobOffer Management Detail Component', () => {
      ...
      providers: [
        ...
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./job-offer-detail').then(m => m.JobOfferDetail),
              resolve: { jobOffer: () => of({ id: 9246 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load jobOffer on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', JobOfferDetail);

      // THEN
      expect(instance.jobOffer()).toEqual(expect.objectContaining({ id: 9246 }));
    });
  });
