...
import { sampleWithRequiredData } from '../candidate-profile.test-samples';
...
describe('CandidateProfile Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 25911 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.candidateProfiles()[0]).toEqual(expect.objectContaining({ id: 25911 }));
  });

  describe('trackId', () => {
    it('should forward to candidateProfileService', () => {
      const entity = { id: 25911 };
      vitest.spyOn(service, 'getCandidateProfileIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCandidateProfileIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.candidateProfilesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
