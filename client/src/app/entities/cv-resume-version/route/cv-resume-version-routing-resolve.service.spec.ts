...
import cvResumeVersionResolve from './cv-resume-version-routing-resolve.service';

describe('CvResumeVersion routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICvResumeVersion returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          cvResumeVersionResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(cvResumeVersionResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
