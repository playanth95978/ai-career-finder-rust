...
import { sampleWithRequiredData } from '../radar-hit.test-samples';
import { RadarHitService } from '../service/radar-hit.service';

import { RadarHit } from './radar-hit';

vitest.useFakeTimers();

describe('RadarHit Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: RadarHit;
  let fixture: ComponentFixture<RadarHit>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(RadarHit);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 25582 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.radarHits()[0]).toEqual(expect.objectContaining({ id: 25582 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 25582 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.radarHits()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 25582 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to radarHitService', () => {
      const entity = { id: 25582 };
      vitest.spyOn(service, 'getRadarHitIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRadarHitIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.radarHitsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
