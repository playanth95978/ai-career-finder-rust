...
describe('Conversation Management Detail Component', () => {
      ...
      providers: [
        ...
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./conversation-detail').then(m => m.ConversationDetail),
              resolve: { conversation: () => of({ id: 5450 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load conversation on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', ConversationDetail);

      // THEN
      expect(instance.conversation()).toEqual(expect.objectContaining({ id: 5450 }));
    });
  });
