...
import offerTailoredResumeResolve from './offer-tailored-resume-routing-resolve.service';

describe('OfferTailoredResume routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IOfferTailoredResume returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          offerTailoredResumeResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(offerTailoredResumeResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
